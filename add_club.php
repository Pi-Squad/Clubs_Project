...
// Récupérer les domaines pour la liste déroulante
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nom === '' || $domain_id <= 0) {
        ...
        $stmt = $pdo->prepare("INSERT INTO club (nom, Domain_id) VALUES (?, ?)");
    ...
    <form method="post">
        ...
        <select id="domain_id" name="domain_id" required>
            ...
            <?php foreach ($domaines as $d): ?>
                <option value="<?= $d['id'] ?>" <?= (isset($_POST['domain_id']) && $_POST['domain_id'] == $d['id']) ? 'selected' : '' ?>>
